...
import { DTA1_PNG, DTA2_PNG, DTA3_PNG } from './fixtures/expected-result.js';
//expect.soft
...
describe('OCR extraction from manual conversion to image', () => {
  ...
  it('OCR with dta2-png.png to have all fields matched', async () => {
    const pdfFile = path.join(inputDir, 'dta2-png.pdf');
    const result = await tryOCRExtraction(pdfFile);
    expect(result.data).toMatchObject(DTA2_PNG);
  });
  it('OCR with dta3-png.png to have all fields matched', async () => {
